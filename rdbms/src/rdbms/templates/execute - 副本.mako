    ...
    <form class="form-horizontal">
      <div class="control-group">
        ...
        <div class="controls" id="columns_1">
		   ...
		   <a class="btn" data-bind="click:delete_column"><i class="fa fa-minus-circle"></i> Delete Column</a>
        </div>
      </div>
    </form>
  </div>
...
<div id="addIndexModal" class="modal hide fade">
  <div class="modal-header">
    <a href="#" class="close" data-dismiss="modal">&times;</a>
    <h3>添加索引</h3>
  </div>
  <div class="modal-body">
    <form class="form-horizontal">
      <div class="control-group">
        <label class="control-label">索引名称</label><input  data-bind="value: $root.index.indexName" id="indexName" type="text" class="input-xlarge">
        <div class="controls">
		   <ul class='unstyled' id='indexs'></ul>
        </div>
      </div>
    </form>
  </div>
  <div class="modal-footer">
    <button class="btn" data-dismiss="modal">${_('Cancel')}</button>
    <button data-bind="click:modalAddIndex" class="btn btn-primary">创建索引</button>
  </div>
</div>

<div id="watchIndexModal" class="modal hide fade">
  <div class="modal-header">
    <a href="#" class="close" data-dismiss="modal">&times;</a>
    <h3>查看索引</h3>
  </div>
  <div class="modal-body">
    <form class="form-horizontal">
      <div class="control-group">
        <div class="controls">
		   <ul class='unstyled' id='indexs_1'></ul>
        </div>
      </div>
    </form>
  </div>
  <div class="modal-footer">
    <button class="btn" data-dismiss="modal">${_('Cancel')}</button>
    <button data-bind="click:modalAddIndex" class="btn btn-primary">保存</button>
  </div>
</div>
...
<script type="text/javascript" charset="utf-8">
  ...
  function ddl_addIndex(table) {
    viewModel.index.tableName(table);
  ...
  function ddl_watchIndex(table) {
    viewModel.index.tableName(table);
	viewModel.index.server(viewModel.server().name());
	viewModel.watchIndex();
//    $('#watchIndexModal').modal('show');
  }
  function add_column(){
  ...
  function delete_column(obj){
        //$('#columns').find("ul").append("<li><input  type='text' class='col' name='columnValues[]'><select class='col_type' name='types[]'><option value='int'>int</option><option value='string'>String</option><option value='char'>char</option><option value='bigint'>bigint</option><option value='float'>float</option></select>是否主键:Yes:<input class='col_checkbox' type=checkbox value='yes' name='iskey[]'/></li>");
		if($('#columns_1').find("ul li").size()>0){
			//var index=$('#columns_1').find("ul li").size();
			//for(var i=0;i<index;i++){$('#columns_1').find("ul li")[i].remove();}
			$('#columns_1').find("ul li").remove();
		}
 }
  ...
  function modalDeleteTable() {
   ...
   function modalAddIndex() {
	viewModel.index.server(viewModel.server().name());
	viewModel.index.chosencolumns.removeAll();
	for (var i=0;i<$('.index').size();i++)
	{
		if($('.index_isChosen')[i].checked){
			viewModel.index.chosencolumns.push(
				{
					'columnName':$('.colName')[i].text,
					'isChosen':$('.index_isChosen')[i].checked,
					//'isInclude':$('.index_isIncluded')[i].checked,
					'lengths':$('.index_length')[i].value
				}
			)
		}
	}
 ...
 //   $("#navigatorLoader").show();
      ...
      $(data.split(" ")).each(function (cnt, table) {
        if ($.trim(table) != "") {
          var _table = $("<li>");
          _table.html("<a href='#' title='" + table + "'><i class='fa fa-table'></i> " + table + "</a><button onclick='ddl_deleteTable(\"" + table + "\")' type='button' class='btn'>删除表</button><button onclick='ddl_addColumn(\"" + table + "\")' type='button' class='btn'>增加列</button><button onclick='ddl_watchIndex(\"" + table + "\")' type='button' class='btn'>查看索引</button><button onclick='ddl_addIndex(\"" + table + "\")' type='button' class='btn'>增加索引</button><ul class='unstyled'></ul>");
