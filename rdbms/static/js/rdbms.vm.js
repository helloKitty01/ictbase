...
function RdbmsViewModel() {
  ...
  self.watchIndex = function() {
	var data = ko.mapping.toJS(self.index);
	$(document).trigger('execute.query', data);
    var request = {
      url: 'http://10.60.1.149:4567/datasources/'+self.index.server()+'/'+self.index.tableName()+'/index?userid=1',
      dataType:'jsonp',
	  jsonp:'jsonpcallback',
	  jsonpcallback:'skycallback',
      type: 'GET',
...
//		self.updateDatabases(self.databases()[0]);
   ...
   self.addIndex = function() {
	...
	var request = {
      url: 'http://10.60.1.149:4567/createindex?userid=1',
      dataType:'jsonp',
	  jsonp:'jsonpcallback',
	  jsonpcallback:'skycallback',
      type: 'GET',
